...
def merging_maxmind():
    ...
    merge_asn_ip(Root.MAXMIND_NAME_LIST[0], Root.MAXMIND_MERG_NAME_LIST[0], Root.MAXMIND_MERG_NAME_LIST[2], Root.IP_VER_4)
    merge_asn_ip(Root.MAXMIND_NAME_LIST[1], Root.MAXMIND_MERG_NAME_LIST[1], Root.MAXMIND_MERG_NAME_LIST[3], Root.IP_VER_6)


def merge_asn_ip(jsonfile1, jsonfile2, output, ipv):
    import json

    js1 = json_to_dict(jsonfile1, gen_data_dir(Root.PREPROCESSED))
    js2 = json_to_dict(jsonfile2, gen_data_dir(Root.PROCESSED))
    ...
    with open(gen_data_dir(Root.ACCUMULATED) + 'Subnets_unmatched' + str(ipv), 'w') as f:
